...
**最后更新**: 2025-09-25
**更新内容**:
- ✅ **设置页面毛玻璃效果修复**: 解决设置页面与首页/书库页毛玻璃效果不一致的问题
  - 统一毛玻璃参数配置：模糊强度5.0→15.0，透明度0.95→0.3
  - 重构页面架构：移除页面级毛玻璃实现，统一使用包装器模式
  - 增强`_SettingsPageWrapper`：添加统一的毛玻璃顶栏支持
  - 使用`GlassEffectConfig`集中管理所有毛玻璃参数
  - 修复与状态栏相关的显示异常问题
- 添加毛玻璃效果问题解决方案到CLAUDE.md开发指南
- 更新历史修复问题记录
