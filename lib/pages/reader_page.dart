...
import 'cover_pagination_page.dart';
import '../widgets/toc_widget.dart';
import '../widgets/side_toast.dart';

part 'reader_page_toolbar_actions_part.dart';
...
List<_ContentElement> _parseMixedContent(String content) {
  ...
  for (var match in imgPattern.allMatches(content)) {
    ...
    if (imagePath != null && imagePath.isNotEmpty) {
      ...
      final heightMatch = RegExp(r'data-height="([^"]+)"').firstMatch(tagText);
...
String _normalizeTitleKey(String title) {
  return title.toLowerCase().replaceAll(RegExp(r'[^\w\u4e00-\u9fff]'), '');
}
...
class _ReaderPageState extends ConsumerState<ReaderPage>
    ...
    if (state == AppLifecycleState.paused ||
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted && !ref.read(toolbarProvider).isVisible) {
          ...
          debugPrint(
              '📱 [Lifecycle] 应用恢复前台，工具栏可见: ${ref.read(toolbarProvider).isVisible}');
        }
      });
    }
  }
  ...
  void _enterImmersiveMode() {
    debugPrint(
        '📱 [Immersive] _enterImmersiveMode 调用，平台: ${Platform.operatingSystem}');
    _hideSystemUI();
    // 只在 iOS 上安排额外的强化调用
    if (Platform.isIOS) {
  ...
  void _showSystemUI() {
    debugPrint(
        '📱 [SystemUI] _showSystemUI 调用，平台: ${Platform.operatingSystem}');
    final settings = ref.read(readerSettingsProvider);
  ...
  void _hideSystemUI() {
    debugPrint(
        '📱 [SystemUI] _hideSystemUI 调用，平台: ${Platform.operatingSystem}');
    if (Platform.isAndroid) {
...
class _ReaderToolbar extends ConsumerWidget {
  ...
  // 工具栏动作/弹窗方法已拆分到 reader_page_toolbar_actions_part.dart
