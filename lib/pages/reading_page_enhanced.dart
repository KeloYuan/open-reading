...
class _ReadingPageEnhancedState extends State<ReadingPageEnhanced> {
  ...
  PageAnimationType _currentAnimationType = PageAnimationType.slide; // 当前翻页动画
      ...
      if (_paginationParams != null) {
        ...
        if (mounted && !_isDisposed) {
          setState(() {
            _pages = pages;
            // 安全地设置当前页面索引
            final targetPageIndex = widget.book.currentPage;
            _currentPageIndex = targetPageIndex.clamp(
              0,
  ...
  /// 简单分页作为回退方案
  void _fallbackSimplePagination() {
    const charsPerPage = 800; // 简单的每页字符数
    final pages = <String>[];

    for (int i = 0; i < _bookContent.length; i += charsPerPage) {
      final end = (i + charsPerPage).clamp(0, _bookContent.length);
      pages.add(_bookContent.substring(i, end));
    }

    if (mounted) {
      setState(() {
        _pages = pages.isEmpty ? ['内容为空'] : pages;
        _currentPageIndex = widget.book.currentPage.clamp(0, _pages.length - 1);
      });
    }
  }
  ...
  void _onPageChangedWithDebounce(int index) {
    ...
    // 立即更新页面索引以确保UI响应，并添加边界检查
    if (mounted) {
      setState(() {
        _currentPageIndex = index.clamp(
          0,
          _pages.isNotEmpty ? _pages.length - 1 : 0,
        );
  ...
  Widget _buildMainContent() {
    ...
    if (shouldShowDoublePage) {
      ...
      return Container(
        color: _currentTheme.backgroundColor,
        child: PageAnimationManager.createPageGestureDetector(
          onPreviousPage: _goToPreviousPage,
          onNextPage: _goToNextPage,
          onToggleControls: _toggleControlsVisibility,
          child: PageView.builder(
            controller: _pageController,
            itemCount: _pages.length,
            itemBuilder: (context, index) => _buildPageWithAnimation(index),
            onPageChanged: (index) {
  ...
  /// 根据动画类型获取PageView的物理属性
  ScrollPhysics _getPageViewPhysics() {
    switch (_currentAnimationType) {
      case PageAnimationType.scroll:
        return const BouncingScrollPhysics(); // 滚动模式使用弹性物理
      case PageAnimationType.none:
        return const NeverScrollableScrollPhysics(); // 无动画时禁用滑动
      default:
        return const ClampingScrollPhysics(); // 其他模式使用标准物理
    }
  }

  /// 构建带动画的页面
  Widget _buildPageWithAnimation(int index) {
    final pageWidget = _buildPageWidget(index);

    // 如果是无动画模式，直接返回页面
    if (_currentAnimationType == PageAnimationType.none) {
      return pageWidget;
    }
  ...
  Widget _buildDoublePageView() {
    return Container(
      ...
      child: PageView.builder(
        ...
        onPageChanged: (index) {
          if (mounted) {
            final newPageIndex = (index * 2).clamp(0, _pages.length - 1);
  ...
  void _showSettingsPanel() {
    showModalBottomSheet(
      ...
      builder: (context) => _buildBottomSettingsPanel(),
    );
  }

  Widget _buildBottomSettingsPanel() {
    return AnimatedContainer(
      duration: const Duration(milliseconds: 300),
      curve: Curves.easeInOut,
