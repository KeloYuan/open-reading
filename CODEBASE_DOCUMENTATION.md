...
- `lib/services/pagination/` 属于旧分页链路，理解历史包袱和兼容逻辑时需要一起看。
...
| `lib/services/pagination/enhanced_paginator_service.dart` | 旧分页服务实现，负责按样式把纯文本切分为页面内容。 | 服务层、Flutter | 旧分页链路 |
| `lib/services/pagination/pagination_cache_service.dart` | 分页缓存服务，负责把分页结果按布局签名落盘缓存。 | 服务层、Path Provider、Crypto 哈希、文件系统、JSON、Flutter | 支撑模块 |
| `lib/services/pagination/pagination_isolate_service.dart` | 分页 isolate 服务，把重计算分页任务放到后台线程。 | 服务层、Isolate、Flutter | 支撑模块 |
| `lib/services/pagination/pagination_services.dart` | 分页模块聚合导出文件，统一暴露分页、缓存和预处理能力。 | 服务层、Barrel Export | 聚合导出 |
| `lib/services/pagination/text_preprocessor_helper.dart` | 文本预处理工具，负责清洗段落、空白和章节前的文本规范化。 | 服务层、Flutter | 支撑模块 |
...
- 如果你想先理解“Web/Foliate 主链路背后的解析与支撑能力”，下一轮可以沿 `foliate_reader_page.dart` → `reader_core/` → `services/pagination/` 对照阅读。
